    ...
    const validation = (req, res, next) => {
        ...
        Object.keys(shemaDonation).forEach((item) => {
            ...
            if (itemShema.type === "email" && !validEmail && email !== undefined) {
                throw new AppError_1.AppError("Esta no formato incorreto, tente um email valido", true);
            }
            if (itemShema.type === "string" &&
                !stringvalid &&
                data[item] !== undefined) {
                // validando string
                types.push(`Campo ${item} esta no formato incorreto ou Vazio! Tente utilizar string. `);
            }
            // validando number
            if (itemShema.type === "number" &&
